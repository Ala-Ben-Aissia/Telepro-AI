...
class EnhancedAnalyticsService:
    ...
    @staticmethod
    def get_engagement_overview(days=30):
        """
        ...
        active_patients = Patient.objects.filter(
            ...
            communicationlog__responded_at__gte=threshold_date
        ).distinct().count()
        ...
        response_rate = responded_logs / total_communications if total_communications > 0 else 0
        ...
        return {
            ...
            "active_percentage": (active_patients / total_patients * 100) if total_patients > 0 else 0,
            "high_engagement_patients": high_engagement_patients,
            "high_engagement_percentage": (high_engagement_patients / total_patients * 100) if total_patients > 0 else 0,
            "low_engagement_patients": low_engagement_patients,
            "low_engagement_percentage": (low_engagement_patients / total_patients * 100) if total_patients > 0 else 0,
    ...
    def get_engagement_trends(days=90, interval="week"):
        ...
        logs_by_interval = logs.annotate(
            interval=trunc_func('sent_at')
        ...
        current_engagement = Patient.objects.filter(
            is_active=True
        ).aggregate(
            avg_score=Avg('engagement_score'),
            high_engagement=Count(Case(When(engagement_score__gte=0.7, then=1))),
            medium_engagement=Count(Case(When(engagement_score__gte=0.3, engagement_score__lt=0.7, then=1))),
    ...
    def get_campaign_performance(campaign_id=None, days=90):
        ...
        for campaign in campaigns_query:
            ...
            segment_info = [
                {
                    "id": segment.id,
                    "name": segment.name,
                    "patient_count": segment.get_patient_count() if hasattr(segment, 'get_patient_count') else 0
                }
            ...
            campaign_metrics.append({
                ...
                "category": campaign.category.name if campaign.category else "Uncategorized",
        ...
        overall_response_rate = responded_all_logs / total_all_logs if total_all_logs > 0 else 0
    ...
    def get_communication_channel_metrics(days=90):
        ...
        )[0] if channel_metrics else None
        
        # Calculate fastest response channel
        fastest_channel = min(
            [(k, v) for k, v in channel_metrics.items() if v["avg_response_time_hours"] > 0],
            key=lambda x: x[1]["avg_response_time_hours"]
        )[0] if any(v["avg_response_time_hours"] > 0 for v in channel_metrics.values()) else None
    ...
    def get_time_of_day_metrics(days=90):
        ...
        for period, (start_hour, end_hour) in time_periods.items():
            # Filter logs by hour
            period_logs = logs.filter(sent_at__hour__gte=start_hour, sent_at__hour__lte=end_hour)
    ...
    def get_dashboard_data(days=30):
        ...
        return {
            ...
            "campaign_performance": EnhancedAnalyticsService.get_campaign_performance(days=days),
            "communication_channels": EnhancedAnalyticsService.get_communication_channel_metrics(days),
