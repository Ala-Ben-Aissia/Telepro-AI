...
def plot_confusion_matrix_side_by_side(
    y_true_train, y_pred_train, y_true_cv, y_pred_cv, labels=None, save_path=None
):
...
def plot_roc_curve_side_by_side(
    y_true_train, y_score_train, y_true_cv, y_score_cv, save_path=None
):
    ...
    fig, axes = plt.subplots(1, 2, figsize=(12, 5))
    axes[0].plot(
        fpr_train, tpr_train, color="darkorange", lw=2, label=f"AUC = {roc_auc_train:.2f}"
    )
    ...
    axes[1].plot(
        fpr_cv, tpr_cv, color="darkorange", lw=2, label=f"AUC = {roc_auc_cv:.2f}"
    )
...
def plot_precision_recall_curve_side_by_side(
    y_true_train, y_score_train, y_true_cv, y_score_cv, save_path=None
):
